...
class NoteControllerTest extends WebTestCase
{
    ...
    /**
     * Test create note via NoteType.
     */
    public function testCreateNote(): void
    {
        // given
        $user = $this->createUser([UserRole::ROLE_USER->value], 'note_creator_form@example.com');
        $this->httpClient->loginUser($user);

        $entityManager = static::getContainer()->get('doctrine.orm.entity_manager');

        $category = new Category();
        $category->setTitle('Form Test Category');
        $entityManager->persist($category);
        $entityManager->flush();

        $noteTitle = 'Fresh Note '.uniqid();
        ...
        $form = $crawler->filter('button[type="submit"], input[type="submit"]')->last()->form([
            'note' => [
                'title' => $noteTitle,
                'comment' => 'This is a test comment generated by PHPUnit',
        ...
        $createdNote = $entityManager->getRepository(Note::class)->findOneBy(['title' => $noteTitle]);
        $this->assertNotNull($createdNote);
        $this->assertEquals($user->getId(), $createdNote->getAuthor()->getId());
    ...
    {
        // given
        $user = $this->createUser([UserRole::ROLE_USER->value], 'note_editor_form@example.com');
        $this->httpClient->loginUser($user);

        $entityManager = static::getContainer()->get('doctrine.orm.entity_manager');

        $category = new Category();
        $category->setTitle('Initial Category');
        $entityManager->persist($category);

        $note = new Note();
        $note->setTitle('Old Title');
        $note->setComment('Old Content');
        $note->setAuthor($user);
        $note->setCategory($category);
        $note->setStatus(NoteStatus::ACTIVE);
        $entityManager->persist($note);
        ...
        $updatedTitle = 'Brand New Title '.uniqid();

        $crawler = $this->httpClient->request('GET', self::TEST_ROUTE.'/'.$note->getId().'/edit');

        // when
        $form = $crawler->filter('button[type="submit"], input[type="submit"]')->last()->form([
            'note' => [
                'title' => $updatedTitle,
                'comment' => 'Updated content here',
                'category' => $category->getId(),
            ],
        ]);
        $this->httpClient->submit($form);
        ...
        $entityManager->clear();
        $refreshedNote = $entityManager->getRepository(Note::class)->find($note->getId());
        $this->assertEquals($updatedTitle, $refreshedNote->getTitle());
        $this->assertEquals('Updated content here', $refreshedNote->getComment());
    }
    ...
    {
        // given
        $user = $this->createUser([UserRole::ROLE_USER->value], 'note_deleter_form@example.com');
        $this->httpClient->loginUser($user);

        $entityManager = static::getContainer()->get('doctrine.orm.entity_manager');

        $category = new Category();
        $category->setTitle('Category for deletion');
        $entityManager->persist($category);

        $note = new Note();
        $note->setTitle('To Be Erased');
        $note->setAuthor($user);
        $note->setCategory($category);
        $note->setStatus(NoteStatus::ACTIVE);
        $entityManager->persist($note);
        ...
        $crawler = $this->httpClient->request('GET', self::TEST_ROUTE.'/'.$noteId.'/delete');
        ...
        $entityManager->clear();
        $deletedNote = $entityManager->getRepository(Note::class)->find($noteId);
